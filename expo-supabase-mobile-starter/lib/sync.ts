...
export const getUnsyncedCount = async (): Promise<number> => {
  ...
  try {
    ...
    for (const table of tables) {
      const count = await queryData(table, ['COUNT(*) as count'], { synced: false })
...
export const performFullSync = async (): Promise<void> => {
  ...
  updateSyncStatus({ isRunning: true, errorCount: 0 })
...
const syncTable = async (table: string): Promise<void> => {
  ...
  try {
    ...
    const unsyncedRecords = await queryData(table, ['*'], { synced: false })
    ...
    updateSyncStatus({ errorCount: syncStatus.errorCount + 1 })
  }
}
...
const processRecord = async (table: string, record: Record<string, unknown>): Promise<void> => {
  try {
    ...
    // Mark as synced
    await updateData(table, { synced: true }, { id: record.id })
      ...
      await executeTransaction([
        async () => {
          for (const record of serverRecords) {
            await updateData(table, { ...record, synced: true }, { id: (record as any).id })
          }
        }
      ])
    }
