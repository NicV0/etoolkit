...
export default function NewInvoiceScreen() {
  ...
  const { control, handleSubmit, formState: { errors, isSubmitting }, setValue } = useForm<InvoiceFormData>({
  ...
  const handleClientSelect = () => {
    router.push({
      ...
      params: { returnTo: '/billing/invoices/new' }
    });
  };
  ...
  return (
    <View className="flex-1 bg-gray-50">
      ...
      <View className="bg-white border-b border-gray-200 px-4 py-3">
        <View className="flex-row items-center">
          ...
          <Text className="text-lg font-semibold text-gray-900">
            {quoteId ? 'Convert Quote to Invoice' : 'New Invoice'}
          </Text>
        </View>
      </View>
      ...
      <ScrollView className="flex-1 p-4">
        <Card>
          <Text className="text-lg font-semibold mb-4">Invoice Information</Text>
        ...
        <View className="mt-6 space-y-3">
          <Button
            ...
            className="w-full"
          >
            {quoteId ? 'Convert to Invoice' : 'Create Invoice'}
          </Button>
