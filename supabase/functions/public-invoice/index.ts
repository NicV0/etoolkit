...
function renderInvoiceHTML(opts: any) {
  ...
  return `<!DOCTYPE html><html><body style="font-family: sans-serif;">
    ...
    <p>Total: $${(opts.totals.subtotal + opts.totals.tax).toFixed(2)}</p>
  </body></html>`;
}
...
Deno.serve(async (req) => {
  try {
    ...
    const { data: link } = await sb
      ...
      .maybeSingle();
    if (!link || link.kind !== 'invoice') return new Response('Not Found', { status: 404 });

    const { data: inv } = await sb.from('invoices').select('*').eq('id', link.entity_id).single();
    const { data: items } = await sb.from('invoice_items').select('*').eq('invoice_id', link.entity_id);
    ...
    const tax = subtotal * ((inv?.tax_percent||0)/100);
